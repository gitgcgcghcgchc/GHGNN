...
from Model_package.LGCN import manifolds
from Model_package.LGCN.coder.decoders import LinearDecoder
from Model_package.LGCN.coder.encoders import LGCN_encoder as encoder
...
class LGCN(nn.Module):
    def __init__(self, args):
        ...
        self.encoder = encoder(args)  # 使用GIL作为Encoder
        # self.decoder = LinearDecoder(args)  # 使用LinearDecoder作为Decoder
        self.manifold = getattr(manifolds, 'Lorentzian')()
        self.c =args.c
        self.manifold_name = args.manifold
        # self.pool = SelfAttentionPooling(args.dim, keep_ratio=args.pooling_ratio, activation=torch.nn.GELU(), args=args, c=self.c)
    ...
    def forward(self, data):
        ...
        # Encoder部分: 提取节点的表示
        emb_node1, encoded_x1 = self.encoder.encode(x1, adj1)

        emb_node2, encoded_x2 = self.encoder.encode(x2, adj2)
