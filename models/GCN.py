...
class GCN(nn.Module):
    def __init__(self, args):
        ...
        for i in range(self.n_layer):
           if i == 0:
             gc_layers.append(GraphConvolution(args.feat_dim, args.dim, args.dropout, act, args.bias))
           else:
             gc_layers.append(GraphConvolution(args.dim, args.dim, args.dropout,act,args.bias))
        ...
        self.mlp_couple1 = nn.Sequential(
            nn.Linear(args.dim * 6, args.dim),  # 第一个全连接层
            nn.GELU(),  # 激活函数
            nn.Dropout(args.dropout),  # Dropout层
            nn.Linear(args.dim, 1)  # 输出层
        )
