...
class PoincareBall(Manifold):
    ...
    def proj(self, x, c):
        # x: [batch_size, m, n] or [m, n]
        norm = x.norm(dim=-1, keepdim=True, p=2).clamp_min(self.min_norm)  # [..., 1]
        maxnorm = (1 - self.eps[x.dtype]) / (c ** 0.5)

        # maxnorm needs to be broadcastable to norm
        maxnorm = torch.as_tensor(maxnorm, dtype=x.dtype, device=x.device)
        while maxnorm.ndim < norm.ndim:
            maxnorm = maxnorm.unsqueeze(0)
    ...
    def expmap(self, u, p, c):
        ...
        u_norm = u.norm(dim=-1, p=2, keepdim=True).clamp_min(self.min_norm)  # [..., 1]
        lambda_p = self._lambda_x(p, c)  # [..., 1]

        # 确保 lambda_p 可以广播乘上 u_norm
        while lambda_p.ndim < u_norm.ndim:
            lambda_p = lambda_p.unsqueeze(-1)

        second_term = (
                tanh(sqrt_c / 2 * lambda_p * u_norm) * u / (sqrt_c * u_norm)
        )  # [..., n]

        gamma_1 = self.mobius_add(p, second_term, c)  # [..., n]
        return gamma_1

    def logmap(self, p1, p2, c):
        sub = self.mobius_add(-p1, p2, c)  # [..., n]
        sub_norm = sub.norm(dim=-1, p=2, keepdim=True).clamp_min(self.min_norm)  # [..., 1]
        lam = self._lambda_x(p1, c)  # [..., 1]
        sqrt_c = c ** 0.5

        # 确保 lam 和 sub_norm 广播一致
        while lam.ndim < sub_norm.ndim:
            lam = lam.unsqueeze(-1)

        scale = (2. / sqrt_c) / lam * artanh(sqrt_c * sub_norm) / sub_norm  # [..., 1]
        return scale * sub  # [..., n]

    def expmap0(self, u, c):
        sqrt_c = c ** 0.5
        u_norm = u.norm(dim=-1, p=2, keepdim=True).clamp_min(self.min_norm)  # [..., 1]
        gamma_1 = tanh(sqrt_c * u_norm) * u / (sqrt_c * u_norm)  # [..., n]
        return gamma_1
    ...
    def mobius_add(self, x, y, c, dim=-1):
        x2 = torch.sum(x ** 2, dim=dim, keepdim=True)  # [..., 1]
        y2 = torch.sum(y ** 2, dim=dim, keepdim=True)  # [..., 1]
        xy = torch.sum(x * y, dim=dim, keepdim=True)  # [..., 1]

        num = (1 + 2 * c * xy + c * y2) * x + (1 - c * x2) * y  # [..., n]
        denom = 1 + 2 * c * xy + c ** 2 * x2 * y2  # [..., 1]

        return num / denom.clamp_min(self.min_norm)  # [..., n]

    def mobius_matvec(self, m, x, c):
        sqrt_c = c ** 0.5
        x_norm = x.norm(dim=-1, keepdim=True, p=2).clamp_min(self.min_norm)  # [..., 1]

        mx = torch.matmul(x, m.transpose(-1, -2))  # [..., n]
        mx_norm = mx.norm(dim=-1, keepdim=True, p=2).clamp_min(self.min_norm)  # [..., 1]

        res_c = tanh(mx_norm / x_norm * artanh(sqrt_c * x_norm)) * mx / (mx_norm * sqrt_c)  # [..., n]

        cond = (mx == 0).prod(-1, keepdim=True, dtype=torch.uint8)  # [..., 1]
        res_0 = torch.zeros(1, dtype=res_c.dtype, device=res_c.device)  # [1]

        res = torch.where(cond.bool(), res_0, res_c)  # [..., n]
        return res
